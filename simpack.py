...
if __name__ == '__main__':
    ...
    while True:
        ...
        if event == 'Run':
            ...
            (x_min, x_max, y_min, y_max) = parseDomain(values['Domain'])
            ...
            lx = (x_max - x_min)
            ly = (y_max - y_min)
            nno_x = int((nno*lx/ly)**0.5) # int((lx/ly)*nno/(1 + (lx/ly)))
            ...
            if sol_met == 'FEM':
                ...
                sim.rhs_ = parseRHS(values['Right hand side'], values['rhs_coeff'].strip())
                sim.bcs_ = parseBC(values)
                if not sim.rhs_:
